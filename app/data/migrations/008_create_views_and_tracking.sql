-- 008_create_views_and_tracking.sql
-- Create migration tracking and helpful views
...
-- Create indexes for the view (only if tables exist)
DO $$
BEGIN
  -- Only create indexes if tables exist
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'daily_total') THEN
    CREATE INDEX IF NOT EXISTS idx_daily_total_user_date_summary ON daily_total(user_id, date DESC);
  END IF;

  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'goal_base') THEN
